...
def get_variables(context, **kw):
    ...
    # ⚡ Ajuste: congelar fecha de muestreo si ya existe
    sample_date = None
    if IAnalysisRequest.providedBy(context):
        date_sampled = getattr(context, "getDateSampled", lambda: None)()
        if date_sampled:
            sample_date = DT2dt(date_sampled)

    variables = {
        "context": context,
        "id": api.get_id(context),
        "portal_type": portal_type,
        "year": (sample_date and sample_date.strftime("%y")) or get_current_year(),
        "yymmdd": (sample_date and sample_date.strftime("%y%m%d")) or get_yymmdd(),
        "parent": parent,
        "seq": 0,
        "alpha": Alphanumber(0),
    }
